# BaseUIRenderBridge 概览

## 背景

当前 BaseUI 客户端体系由三层协作构成：

- `BaseUIClientScreen`：Screen 封装与调用入口
- `BaseUIRenderBridge`：帧级渲染协调层
- `BaseUIElement`：retained tree（组件树、布局、输入分发、递归渲染）

其中 `BaseUIRenderBridge` 的定位不是“新组件基类”，而是连接 Screen 生命周期与 UI 树执行语义的桥接层。

---

## 核心定位

`BaseUIRenderBridge` 是：

- Screen/Forge 到 UI 树的统一渲染入口
- 帧级上下文与生命周期协调边界
- 运行时容错与状态收尾的执行点

`BaseUIRenderBridge` 不是：
...
- 定义独立 `BaseUIContext` 契约（graphics/mouse/partialTick/size/debug）
...
## 与 BaseUIClientScreen 的关系

`BaseUIClientScreen` 负责对调用方提供统一 API 与 Screen 壳行为；
`BaseUIRenderBridge` 负责屏幕运行期间的帧级协调与执行语义。
